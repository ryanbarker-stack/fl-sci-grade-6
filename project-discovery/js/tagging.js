...
(function(P){
  ...
  P.idValid=id=>typeof id==='string'&&new RegExp(P.config.idPattern).test(id);
  ...
  P.validate=function(s){
    const errors=[];
    if(!s||!P.questions[s.grade]||!P.idValid(s.classOnlyId))return [{id:'identity',message:'Check your Class-Only ID and grade.'}];
  ...
  P.cleanImport=function(raw){
    ...
    if(raw.schemaVersion!==P.config.schemaVersion||raw.questionnaireVersion!==P.config.questionnaireVersion)throw Error('This draft uses a different questionnaire version. Keep the file and ask your teacher for the matching website.');
    if(!P.questions[raw.grade]||!P.idValid(raw.classOnlyId))throw Error('This draft has an invalid grade or Class-Only ID.');
    ...
    return {grade:raw.grade,classOnlyId:raw.classOnlyId,period:raw.period,answers:a};
  };
  P.tags=function(s){
